...
async def take_order_handler(callback: types.CallbackQuery, bot: Bot):
    ...
    try:
        ...
        # 5. Перевірка що саме ми захопили
        ...
        # 6. Будуємо текст З НУЛЯ з html.escape — 100% чистий HTML
        short_id    = str(order_id)[:6].upper()
        ...
        safe_name    = _html.escape(taker_name)
        ...
        text = (
            ...
            f"📍 <b>Адреса:</b> {safe_address}\n"
            f"👤 <b>Клієнт:</b> {safe_client}\n"
            f"📞 <b>Телефон:</b> {safe_phone}\n"
            f"{pay_icon} <b>Сума:</b> {order.get('amount', '0')} {currency} ({pay_type_str})\n"
        )
        if safe_comment:
            text += f"\n💬 <b>Коментар:</b> <i>{safe_comment}</i>\n"
...
async def uber_close_handler(callback: types.CallbackQuery, bot: Bot):
    ...
    try:
        ...
        # 5. Будуємо фінальний текст з нуля
        final_text = (
            f"🛵 <b>Замовлення #{short_id}</b>\n\n"
            f"📍 <b>Адреса:</b> {safe_address}\n"
            f"👤 <b>Клієнт:</b> {safe_client}\n"
