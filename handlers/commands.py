...
await message.answer(*(lang, ‘dont_understand’))
...
await state.clear()
context = await db.get_user_context(message.from_user.id)
await message.answer(*(lang, ‘staff_added’, name=name, role=*(lang, ‘role_c’) if data.get(‘joining_role’, ‘courier’) == “courier” else *(lang, ‘role_m’)))
await show_main_menu(message, context)
except Exception as e:
logger.error(f”Помилка додавання персоналу: {e}”)
await message.answer(*(lang, ‘staff_add_err’))
