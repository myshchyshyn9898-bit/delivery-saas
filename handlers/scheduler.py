...
# Виправлення Gemini: перевіряємо тільки замовлення за останні 24 години
# щоб не накопичувати “примарні” замовлення з минулого
...
```
    ...
    for order in res.data:
        ...
        if now > deadline:
            ...
            # 3. Відправляємо всім менеджерам закладу
            managers_res = await db._run(
                lambda: db.supabase.table("staff").select("user_id").eq("business_id", biz_id).eq("role", "manager").execute()
            )
